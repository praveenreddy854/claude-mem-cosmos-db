...
Optional shared-memory mode keeps SQLite as the local cache and synchronizes sessions, prompts, observations, and summaries through Azure Cosmos DB so multiple machines can share the same memory corpus.
