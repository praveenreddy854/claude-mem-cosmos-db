...
Install Claude-Mem Cosmos DB directly from the plugin marketplace:
...
> **Important:** Claude-Mem Cosmos DB is published on npm, but running `npm install -g claude-mem-cosmos-db` installs the
> **SDK/library only**. It does **not** register plugin hooks or start the worker service.
> To use it as a persistent memory plugin, always install via the `/plugin` commands above.
...
See [CHANGELOG](https://github.com/praveenreddy854/claude-mem-cosmos-db/blob/main/CHANGELOG.md) for complete version history.
