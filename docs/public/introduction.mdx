...
Claude-Mem Cosmos DB uses the original Claude-Mem project as its base and extends it with Azure Cosmos DB-backed shared memory while preserving Claude-Mem's session memory workflow.

> This documentation covers the `claude-mem-cosmos-db` fork, which is based on [`thedotmack/claude-mem`](https://github.com/thedotmack/claude-mem) and maintained by [`praveenreddy854`](https://github.com/praveenreddy854).
